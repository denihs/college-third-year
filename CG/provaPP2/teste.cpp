...
// g++ -o 4.60 4.60.cpp -lglut -lGL -lGLU -lm
// Sumanta Guha.
//////////////////////////////////////////////////////////////////////////////////////////////
...
vector<Man>::iterator manVectorIteratorTwo;
vector<Man>::iterator manVectorAnimationIteratorTwo;
...
void drawScene(void) {
    ...
    if (!animateMode)
    {
        ...
        manVectorIteratorTwo->writeData();
    }
    ...
    vector<Man>::iterator localManVectorIteratorTwo;
    ...
    if (!animateMode) // Develop mode.
    {
        ...
        localManVectorIteratorTwo = manVector.begin();
        ...
        while(localManVectorIterator != manVector.end() )
        {
            ...
            localManVectorIteratorTwo->draw();
            localManVectorIteratorTwo++;
        }
        ...
        // use separate iterator so as to leave develop mode iterator unchanged.
    {
        ...
        manVectorAnimationIteratorTwo->draw();
    }
...
void outputConfigurations(void)
{
    ...
    while(localManVectorIteratorTwo != manVector.end() )
    {
        localManVectorIteratorTwo->outputData();
        localManVectorIteratorTwo++;
    }
    outFile.close();
}
...
void setup(void)
{
    ...
    manVectorIteratorTwo = manVector.begin();

    manVectorAnimationIterator = manVector.begin();
    manVectorAnimationIteratorTwo = manVector.begin();
...
void keyInput(unsigned char key, int x, int y)
{
    ...
    switch(key)
    {
        ...
        case 'n': // Create new man configuration.
            ...
            manVectorIteratorTwo->setHighlight(0);

            localManVectorIterator = manVectorIterator;
            localManVectorIteratorTwo = manVectorIterator;
            manVectorIterator++;
            manVectorIteratorTwo++;

            // Insert copy of current configuration and highlight.
            manVectorIterator  = manVector.insert(manVectorIterator, Man(*localManVectorIterator) );

            manVectorIteratorTwo = manVector.insert(manVectorIteratorTwo, Man(*localManVectorIteratorTwo) );

            manVectorIterator->setHighlight(1);
            manVectorIteratorTwo->setHighlight(1);

            glutPostRedisplay();
            break;
        case ' ': // Select next body part.
            manVectorIterator->incrementSelectedPart();
            manVectorIteratorTwo->incrementSelectedPart();
        ...
        case 9:
            ...
            if (manVectorIteratorTwo == manVector.end() )
                manVectorIteratorTwo = manVector.begin();
        ...
        case 8:
            if (manVectorIterator != manVector.begin() ) // Not first configuration.
            {
                ...
                localManVectorIteratorTwo = manVectorIteratorTwo;
                localManVectorIteratorTwo--;
                ...
                manVectorIteratorTwo = manVector.erase(manVectorIteratorTwo);
            ...
            else // First configuration
            {
                ...
                manVector.insert(manVectorIteratorTwo, Man() );
            }
        ...
        case 127:
            if (manVector.size() > 1)
            {
                ...
                if (manVectorIteratorTwo != manVector.begin() )
                    manVectorIteratorTwo--;
...
void specialKeyInput(int key, int x, int y) {
    ...
    if (key == GLUT_KEY_PAGE_DOWN) manVectorIteratorTwo->decrementPartAngle();
    if (key == GLUT_KEY_PAGE_UP) manVectorIteratorTwo->incrementPartAngle();
    if (key == GLUT_KEY_LEFT) manVectorIteratorTwo->decrementForwardMove();
    if (key == GLUT_KEY_RIGHT) manVectorIteratorTwo->incrementForwardMove();

    if (key == GLUT_KEY_DOWN)
    {
        if (!animateMode) manVectorIterator->decrementUpMove();
        if (!animateMode) manVectorIteratorTwo->decrementUpMove();
        else animationPeriod +=10;
    }
    if( key == GLUT_KEY_UP)
    {
        if (!animateMode) manVectorIterator->incrementUpMove();
        if (!animateMode) manVectorIteratorTwo->incrementUpMove();
