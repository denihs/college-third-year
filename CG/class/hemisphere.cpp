...
void drawScene(void)
{
   ...
   for(j = 0; j < q; j++)
   {
      ...
      glBegin(GL_TRIANGLE_STRIP);
         for(i = 0; i <= p; i++)
		 {
            glVertex3f( R * a * cos( (float)(j+1)/q * PI/2.0 ) * cos( 2.0 * (float)i/p * PI ),
                        R * c * sin( (float)(j+1)/q * PI/2.0 ),
					    R * b * cos( (float)(j+1)/q * PI/2.0 ) * sin( 2.0 * (float)i/p * PI ) );
            glVertex3f( R * a * cos( (float)j/q * PI/2.0 ) * cos( 2.0 * (float)i/p * PI ),
                        R * c * sin( (float)j/q * PI/2.0 ),
					    R * b * cos( (float)j/q * PI/2.0 ) * sin( 2.0 * (float)i/p * PI ) );         
		 }
      glEnd();
   }
